...
class TestQuadraticProblemNLP(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._initial_cost_handmade = (
            0.5 * np.linalg.norm(cls._initial_residual_handmade) ** 2
        )

        # Computing the principal cost
        cls._principal_cost = cls._QP._principal_cost
        cls._principal_cost_handmade = (
            0.5 * np.linalg.norm(cls._principal_residual_handmade) ** 2
        )

        # Variables for computing the derivatives of the residuals
        cls._QP._compute_derivative_residuals()

        cls._derivative_principal_residuals = (
            cls._QP._compute_derivative_principal_residuals()
        )
    ...
    def test_get_q_iter_from_Q(self):
        ...
        self.assertTrue(
            np.array_equal(self._QP._get_q_iter_from_Q(0), self._q_init),
    ...
    def test_get_difference_between_q_iter(self):
        ...
        self.assertTrue(
            np.array_equal(
                self._QP._get_difference_between_q_iter(0), self._q_inter - self._q_init
            ),
            msg=" The difference between the q_iter and q_iter+1 is false",
        )
    ...
    def test_distance_endeff_target(self):
        ...
        self.assertAlmostEqual(
            np.linalg.norm(self._QP._distance_endeff_target(self._q_target)),
            0,
            msg="Error while computing the distance between the end effector and the target",
        )

    def test_initial_residual(self):
        """Testing whether the initial residual is 0 as it should be"""
        self.assertTrue(
            np.array_equal(self._initial_residual_handmade, self._initial_residual)
        )
    ...
    def test_compute_principal_residual(self):
        ...
        self.assertTrue(
            np.array_equal(self._principal_residual, self._principal_residual_handmade),
    ...
    def test_residual_derivative_finite_difference(self):
        ...
        self.assertAlmostEqual(
            np.linalg.norm(fdm_res - self._derivative_residuals),
            0,
            places=6,
    ...
    def test_gradient_finite_difference(self):
        ...
        self.assertAlmostEqual(
            np.linalg.norm(self._gradient - self._gradient_numdiff),
            0,
            places=5,
    ...
    def test_hessian_finite_difference(self):
        ...
        self.assertAlmostEqual(
            np.linalg.norm(self._hessval - self._hessval_numdiff),
            0,
            places=5,
            msg=" The hessian is not the same as the finite difference one",
        )
